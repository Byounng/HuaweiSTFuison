...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        with tempfile.TemporaryDirectory(prefix="pascal_voc_eval_") as dirname:
            ...
            recalls_base = defaultdict(list)  
            recalls_novel = defaultdict(list)  
        ...
        mRecall = {iou: np.mean(x) for iou, x in recalls.items()}  
        ...
        if exist_base:
            ...
            mRecall_base = {iou: np.mean(x) for iou, x in recalls_base.items()}  
            mF1 = {iou: 2 * (mAP[iou] * mRecall[iou]) / (mAP[iou] + mRecall[iou]) for iou in aps.keys()}
            ret["bbox"].update(
                {"bAP": np.mean(list(mAP_base.values())), 
                 "bAP50": mAP_base[50],
                 "bAP75": mAP_base[75],
                 "bRecall": np.mean(list(mRecall_base.values())),  
                 "bRecall50": mRecall_base[50],  
                 "bRecall75": mRecall_base[75],   
                "bF1": np.mean(list(mF1.values())),  
                "bF150": mF1[50],                    
                "bF175": mF1[75]                   
        ...
        if exist_novel:
            ...
            mRecall_novel = {iou: np.mean(x) for iou, x in recalls_novel.items()}  
            ...
            ret["bbox"].update({
                ...
                "nAP50": mAP_novel[50],
                "nAP75": mAP_novel[75],
                "nRecall": np.mean(list(mRecall_novel.values())),  
                "nRecall50": mRecall_novel[50],  
                "nRecall75": mRecall_novel[75],   
                "nF1": np.mean(list(mF1.values())),  
                "nF150": mF1[50],                    
                "nF175": mF1[75]  
            })
