...
class SupConLossWithPrototype(nn.Module):
    ...
    def forward(self, features, labels, protos, proto_labels):
        ...
        features = features[fg_index]  
        labels = labels[fg_index]      
        numel = features.shape[0]      

        
        base_index = torch.eq(labels, proto_labels.reshape(-1,1)).any(axis=0)  
        novel_index = ~base_index  
        if torch.sum(novel_index) > 1:
            ni_pk = torch.div(torch.matmul(features[novel_index], protos.T), self.temperature)  
            ni_nj = torch.div(torch.matmul(features[novel_index], features[novel_index].T), self.temperature)  
            novel_numer_mask = torch.ones_like(ni_nj)  
            novel_numer_mask.fill_diagonal_(0)
            exp_ni_nj = torch.exp(ni_nj) * novel_numer_mask  
        ...
        if torch.any(base_index):
            bi_pi = torch.div(torch.einsum('nc,nc->n', features[base_index], protos[labels[base_index]]), self.temperature) 
            bi_nk = torch.div(torch.matmul(features[base_index], features[novel_index].T), self.temperature)  
            bi_pk = torch.div(torch.matmul(features[base_index], protos.T), self.temperature)  
