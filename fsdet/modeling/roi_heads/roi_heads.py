    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        Args:
            ...
            targets (list[Instances], optional): length `N` list of `Instances`s. The i-th
                ...
                - gt_classes: the label for each instance with a category ranging in [0, 
...
class MoCoROIHeadsV1(StandardROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        '''
        q_box_features = self.box_pooler(features, [p.proposal_boxes for p in proposals]) 
        q_box_features = self.box_head_q(q_box_features) 
        ...
        moco_logits = torch.mm(q, self.queue.clone().detach()) 
...
class DoubleHeadROIHeads(StandardROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        if self.contrastive_branch:
            box_cls_feat_contrast = self.encoder(box_cls_feat)  
...
class ContrastiveROIHeadsWithPrototype(StandardROIHeads):
    def __init__(self, cfg, input_shape):
        ...
        prototype_tensor = torch.load(self.prototype_path)  
        prototype_tensor = prototype_tensor[:-1, :]  
        if self.prototype_dataset == 'PASCAL VOC':
            assert prototype_tensor.shape == (15, 1024)
            prototype_label = torch.arange(15)
        else:
            raise(NotImplementedError, 'prototype not implemented for non-VOC datasets')
