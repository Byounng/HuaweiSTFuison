...
class WeightedAttentionFusion(nn.Module):
    def __init__(self, input_dim):
        ...
        # 初始化权重参数为标量
        # self.attn_weight_box = nn.Parameter(torch.rand(1), requires_grad=True)
        # self.attn_weight_fused = nn.Parameter(torch.rand(1), requires_grad=True)
    ...
    def forward(self, box_features, fused_features):
        ...
        # 拼接特征
        combined_features = torch.cat((weighted_box, weighted_fused), dim=1) #这个东西已经混合了原有的Box特征了
        # combined_features = weighted_fused  #
        # 使用全连接层融合
...
class TSFigureBackBone(nn.Module):
    ...
    def forward(self, features, src_feat=None):
        ...
        for i, (conv, feat, src) in enumerate(zip(self.convs, features, src_feat)):
            ...
            #     copy_feat = feat_out
            # else:
            #     feat_out = F.interpolate(copy_feat, size=src_feat[src].shape[2:], mode='bilinear', align_corners=False)
...
def images_to_tensor(image_paths):
    ...
    for image_path in image_paths:
        # 读取并转换每个图像
        image = Image.open(image_path).convert("RGB")   #L
